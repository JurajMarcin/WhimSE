...
class DistPolicyExplorer(PolicyExplorer):
    ...
    ) -> Iterable[tuple[PolicyModule, PolicyModuleSource]]:
        modules_by_source: dict[PolicyModuleSource, set[PolicyModule]] = {}
        for module, source in modules:
            if module.files and source.source_package:
                modules_by_source.setdefault(source, set()).add(module)
            else:
                yield module, source
        ...
        for fetch_method in self._config.module_fetch_methods:
            if fetch_method == ModuleFetchMethod.LOCAL_MODULE:
                for source, source_modules in modules_by_source.items():
                    ...
                    for module in source_modules.copy():
                        ...
                        try:
                            ...
                            _logger.warning(
                                "Could not fetch local file %r from module %r, will try to use another method",
                                ex.filename,
                                module,
                            )
            else:
                for source, source_modules in modules_by_source.items():
                    ...
                    assert (
                        files
                    ), f"Modules without files should have been already yielded {source=} {source_modules=} {files=}"
                    try:
    ...
    def _get_dist_modules(self) -> dict[PolicyModule, PolicyModuleSource]:
        return dict(
            self._fetch_dist_files(self._package_manager.find_selinux_modules())
        )
